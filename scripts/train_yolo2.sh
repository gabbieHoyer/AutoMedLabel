...
#SBATCH --nodelist=juno
#SBATCH --partition=dgx
#SBATCH --gres=gpu:teslav100:6
#SBATCH --ntasks-per-node=6                          # Request 2 tasks (processes) per node
#SBATCH --mem=192G                                    # 32 GB VRAM per GPU; 16 GPUs; 96 CPU Cores; 24 Hrs max wall time
#SBATCH --cpus-per-task=6     
...
CONFIG_NAME="UH2_spine_t1ax_yolo"
