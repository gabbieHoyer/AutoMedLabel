...
def extract_slice_number(img_name):
    """
    Extract the slice number from the image name, which follows the format '{subject_name}-###.npy'.
    Example: 'subject123-015.npy' -> 15
    """
    # Use regex to extract the number part from the string
    match = re.search(r'-(\d{3})\.npy$', img_name)
    if match:
        slice_number = int(match.group(1))  # Extract the number and convert it to integer
        return slice_number
    return None  # Return None if no slice number is found
