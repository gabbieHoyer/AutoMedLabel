...
from src.finetuning import GPUSetup, log_info, Det2SegTester, EvaluationDataModule, load_segmentation_model, load_detection_model
...
def datamodule(cfg, run_path=None):
    ...
    for dataset_name in cfg.get('dataset').keys():
        ...
        voxel_num_thre2d = cfg.get('dataset').get(dataset_name).get('voxel_num_thre2d')
        kernel_size = cfg.get('dataset').get(dataset_name).get('kernel_size')
    ...
    cfg['datamodule']['voxel_num_thre2d'] = voxel_num_thre2d 
    cfg['datamodule']['kernel_size'] = kernel_size
...
def prepare_training_base(trainable_cfg, model_config, weights_path, finetuned_weights, device, det_cfg):
...
def finetune_evaluate_det2seg_impl(cfg):
    # --------------- SET UP ENVIRONMENT --------------- #  
    ...
    model, det_model = prepare_training_base(
    ...
    if sampled_test_loader is not None:
        ...
        sampled_tester = Det2SegTester(
            model=model,
            det_model=det_model,
            test_loader=sampled_test_loader,
            eval_cfg=model_cfg,
            module_cfg=module_cfg,
            det_model_cfg=cfg.get('obj_det'),
...
def finetune_evaluate_det2seg(config):
    """
    ...
    for model_info in cfg.get('models', []):
        ...
        try:
            finetune_evaluate_det2seg_impl(model_config)
        except Exception:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Det2Seg Evaluation setup.")
    parser.add_argument("config_name", help="Name of the YAML configuration file")
    args = parser.parse_args()
    finetune_evaluate_det2seg(args.config_name)
