...
class MaskPrep():
    ...
    def resize_mask(self, mask_data, image_size_tuple:tuple[int,int]= None):
        ...
        def unpad_to_original_size(mask_slice, original_shape):
            height, width = original_shape
            if height > width:
                pad = (height - width) // 2
                unpadded_mask = mask_slice[:, pad:pad + width]
            elif width > height:
                pad = (width - height) // 2
                unpadded_mask = mask_slice[pad:pad + height, :]
            else:
        ...
        if self.ratio_resize:
            ...
            original_shape = mask_data.shape[1:] if len(mask_data.shape) == 3 else mask_data.shape

            # if self.make_square:
            if self.make_square and (original_shape[0] != original_shape[1]):
                ...
                if (original_shape[0] > image_size_tuple[0]) or (original_shape[1] > image_size_tuple[1]):
                    ...
                    dims = len(np.shape(mask_data))
                    if dims == 2:
                        resized_masks = resize_mask_2D(mask_data, adjusted_size)
                    elif dims == 3:
                        resized_masks = []
                        for mask_slice in mask_data:
                            resized_mask = resize_mask_2D(mask_slice, adjusted_size)
                            resized_masks.append(resized_mask)
                        resized_masks = np.array(resized_masks)
...
class ImagePrep():
    ...
    def resize_images(self, image_data: np.ndarray, image_size_tuple:tuple[int, int]=None, n_channels:int=0) -> np.ndarray:
        ...
        def scaled_resize_image_2D(img_slice, image_size_tuple:(int,int)):
            h0, w0 = img_slice.shape[:2]  # Original dimensions
            # Resize long side to imgsz while maintaining aspect ratio
            r = image_size_tuple[0]/ max(h0, w0)  # Ratio
            if r != 1:  # If sizes are not equal
                w, h = (min(math.ceil(w0 * r), image_size_tuple[0]), min(math.ceil(h0 * r), image_size_tuple[0]))
                resized_img = cv2.resize(img_slice, (w, h), interpolation=cv2.INTER_CUBIC)  # cv2.INTER_LINEAR
                return resized_img
            return img_slice
        ...
        if self.ratio_resize:
            ...
            if (dims==2 and n_channels==0) or (dims==3 and n_channels>0):   
                resized_images = scaled_resize_image_2D(image_data, image_size_tuple)
            elif (dims==3 and n_channels==0) or (dims==4 and n_channels>0):
                resized_images = []
                for img_slice in image_data:
                    resized_img = scaled_resize_image_2D(img_slice, image_size_tuple)
...
def get_bounding_boxes(multiclass_mask, instance=False):
    ...
    #                 # If the original mask was padded and then resized to smaller dimensions, unpad it
    #                 if len(resized_masks.shape) == 2:
    #                     resized_masks = unpad_to_original_size(resized_masks, original_shape)
    #                 elif len(resized_masks.shape) == 3:
    #                     resized_masks = np.array([unpad_to_original_size(slice, original_shape) for slice in resized_masks])
